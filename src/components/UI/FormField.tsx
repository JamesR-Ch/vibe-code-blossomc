...
export const FormField = ({ field, value, onChange }: FormFieldProps) => {
  ...
  useEffect(() => {
    if (!isFocused && field.type === 'number' && (field.name === 'price' || field.name === 'depositAmount' || field.name === 'travelFee' || field.name === 'addonPrice1' || field.name === 'addonPrice2' || field.name === 'addonPrice3' || field.name === 'addonPrice4') && value) {
  ...
  const handleFocus = () => {
    setIsFocused(true);
    if (field.type === 'number' && (field.name === 'price' || field.name === 'depositAmount' || field.name === 'travelFee' || field.name === 'addonPrice1' || field.name === 'addonPrice2' || field.name === 'addonPrice3' || field.name === 'addonPrice4' || field.name === 'hours')) {
  ...
  const getInputValue = () => {
    if (field.type === 'number' && field.name === 'hours') {
      if (isFocused) return displayValue;
      return value ? String(value) : '';
    }
    if (field.type === 'number' && (field.name === 'price' || field.name === 'depositAmount' || field.name === 'travelFee' || field.name === 'addonPrice1' || field.name === 'addonPrice2' || field.name === 'addonPrice3' || field.name === 'addonPrice4')) {
          ...
          {field.name === 'hours' ? (
            <>
              <input
                ...
                onBlur={handleBlur}
                placeholder=" "
                className="peer w-full pl-8 sm:pl-10 pr-3 sm:pr-4 pt-6 pb-2 border border-gray-300 rounded-lg sm:rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 hover:border-gray-400 hover:shadow-md text-sm sm:text-base"
                required={field.required}
              />
              <label className="absolute left-8 sm:left-10 top-2 text-[10px] sm:text-xs font-semibold text-blue-600 pointer-events-none transition-all duration-200 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400 peer-placeholder-shown:font-normal peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[10px] peer-focus:font-semibold peer-focus:text-blue-600">
